...
At the heart of empirical factor research sits a straightforward yet stubborn question: "Is this factor's return statistically different from zero, and is it economically meaningful hence likely to persist?". It's no surprise that investors care about more than mean and variance of return when judging factors and associated expected returns. Any additional look at the distibution like tail risk, time-varying parameter, dependnce on the past can translate into a real world edge in investment. Even so, a Gaussian framework, uniquely defined by mean and variance remains a reasonable starting point, both for its theoretical gounding and analytical tractability. That's exactly where we'll start this time.

In this blog, we'll try to unpack the nuts and bolts behind the modeling and evaluation. We begin with the classic and frequentist toolkit used in in empirical asset pricing research and then explore how ensen, Kelly & Pedersen (2023) recommend to add structure and reduce variance on top of it via a Bayesian framework. Even with all the machine learning thecniques in the spotlight, these structured econometric ideas matter when samples are short, dimensions explode, and signals hide in noise, the everyday reality of markets.
