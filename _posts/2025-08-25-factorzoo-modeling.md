...
In this blog, we'll try to unpack the nuts and bolts behind the modeling and evaluation. We start with the classic frequentist toolkit used in in empirical asset pricing research and then explore the Bayesian framework that Jensen, Kelly & Pedersen (2023) recommended to add addtional structure and reduce variance. Even with all the machine learning thecniques in the spotlight, these structured econometric techniques with the market structure that it presents matter when samples are short, dimensions explode, and signals hide in noise, the everyday reality of the majority part of the financial markets.
