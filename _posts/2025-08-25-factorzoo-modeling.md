...
In our previous blog, [A First Glimpse into the "Factor Zoo"](https://skybluerw.github.io/2024/12/07/factor-zoo.html), we took a bird’s-eye view of the empirical evidence and saw how drastically conclusions can swing across datasets and portfolio-construction choices. Replication debates aside, it’s time to step closer and look at the inner workings of factor modeling and evaluation.

At the heart of empirical factor research lies one straightforward yet challenging question: "Is this factor's return statistically different from zero, and is it economically meaningful hence likely to persist?". It's no surprise that investors care more than mean and variance when judging factors and associated expected returns. Any additional look at the distibution like tail risk, time-varying parameter, dependnce on the past can translate into a real word edge in investment. Even so, a Gaussian framework, which is uniquely defined by mean and variance remains a reasonable starting point, both for its theoretical gounding and analytical tractability. That's exactly where we'll start this time.


In this blog, we'll try to unpack the nuts and bolts behind the modeling and evaluation. We begin with the classic, frequentist toolkit with those classic methods in empirical asset pricing research and then move to a Bayesian framwork innovation that Jensen, Kelly & Pedersen (2023) introduced to incorporate additional structures in modeling. Even under the current trend with machine learning techniques gainning more and more attention, these structured econometric insights remain crucial when samples are small, dimensions explode and signals hide in noise - the realities of financial markets for a lot of the times!
