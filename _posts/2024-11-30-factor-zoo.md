...
While, it doesn't mean investors who share similar investing procedures as Jensen et al should celebrate over the promising prospect that can originated from the 82% replication rate. Many factors share common ground in predicting returns and adding highly correlated factors doesn't contribute much additional predictive power. As a matter of fact, Jensen el al grouped all the factors into 13 categories
...
It becomes even trickier when it comes to the difference in data sets. Typically, statistical conclusions from a larger data set should provide more confidence hence Jensen et al's data set should be favored for its long history and wider coverage on financial markets globally. While this edge on confidence rely on the assumption of same distribution across history and markets. It is not always the case in both dimmensions. 
