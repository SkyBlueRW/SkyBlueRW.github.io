...
Ultimately, not weight. but return and risk associated with the portfolio.
...
It should be note that for linear constraints, the corresponding overlay $$x_i$$ bears further economic interpretation as **shadow price weighted characteristic portfolios**. $$A_i$$ is the i-th column of the linear constraints which can be any security characteristic factor exposure, industry membership, etc...
...
$$\dfrac{1}{\lambda} Q^{-1}A_i$$ is actually the characteristic portfolio for $$A_i$$ defined by the following optimization:
