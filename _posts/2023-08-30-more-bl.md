...
The scaling factor $$\tau$$ is all required to determine the unceratainty around prior. $$\tau$$ is generally assigned a small value as the uncertainty around expected return should be smaller than the uncertainty around the return itself (Black & Litterman (1992))
