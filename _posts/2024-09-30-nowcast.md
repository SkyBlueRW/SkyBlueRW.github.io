...
In a previous blog [Nowcasting: The News From Jagged Economic Data](https://skybluerw.github.io/2023/05/25/news-from-ragged-data.html), we explored the remarkable capabilities of a dynamic factor model-based nowcasting system to consume and interpret the irregular flow of macroeconomic data. While even such a powerful model struggles when confronted with the once-in-a-century (if not once-in-a-millennium) crisis - the pandemic.

From a variety of aspects, the Covid recession is unlike economic downturns that human witnessed historically. The recession 
