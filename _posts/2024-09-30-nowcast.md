...
With the updated nowcasting system with significantly more structures placed, the new model was able to acquire significantly improved GDP nowcast (especially during the Covid recession) as imagined. While the improvement does not come with no cost. The relatively light EM (expectation maximization) algorithm previously used is no longer adequote with the new nonlinearity system. 
...
Looking back, previously, we are using a model with invariant paratmers and estimate with rolling window of data to adapt to the change of the market. The assumption is that market would not change that much even in a limited short window of time. The hit of the Coivd Recession kind crush this assumption. With the Covid hit with unprecedented impact economic and swift come and go, previous modeling strategy is no longer working. To deal with the changing market, we have to add a lot of strucutres. Time varying volatility and long term growth trend is added to adapt to the change of the market. acompnaied by the nonliearity often brought by the changing paratmers, we have to use more complex estimation method, which requires more data to work.

We are more and more in a tradeoff of the complexity bounded by the amount of data that we have, and the more and more frequent changing market condition. And with growing geopolitical tension, re-organized global supply chain, it probably implies a new era of more rapid and dramatic market change that requires more flexbility in the model. Making smart choice on where to add the flexibility to your model is probably more and more important.
