...
The final results of all backtests are displayed in the chart at the beginning of the blog post. The detailed performance is also presented in the table below. The "MD @ x Risk" represents the MD portfolio optimized at volatility target of x. As initially anticipated, MD proves to be a reasonable solution for strategic asset allocation. I would like to emphasize two specific points:

1. The volatility target effectively constrains the volatility of all portfolios close to their respective volatility targets. By utilizing only local information during each rebalancing, we are able to manage long-term volatility effectively. This outcome can likely be attributed to the clustering and long-term mean reversion characteristics that are prevalent in volatility.

2. As we increase the volatility target, the final expected return also increases in a relatively stable manner. In my view, this is an indirect indication that the risk is utilized rather efficiently. Every bit of additional risk-taking can yield further rewards.

In my opinion, this MD with risk target approach serves as an excellent starting point for asset allocation, particularly for Asset Only investors without stringent constraints related to matching durations or cash flows. For scenarios requiring different volatility levels, a range of solutions can be provided to accommodate specific product and client demands.
