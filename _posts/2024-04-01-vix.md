...
It's much easier to verify that such a portfolio of options scaled by $$\dfrac{1}{K^2}$$ has a constant vega under the assumption of BSM (Black-Schole-Schole option pricing). While neither of this properoty nor the calculation of VIX depend on BSM.
...
To make the portfolio vega invariant to underlying price, set the partial derivative to 0 leads to the weight proportional to $$\dfrac{1}{K^2}$$. As demonstrated in the Figure 3a from DDKZ(1999), the variance swap can be replicated with inifnite number of options with different strikes appropriately weighted.
