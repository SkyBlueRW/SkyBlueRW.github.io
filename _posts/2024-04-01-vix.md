...
Long in short with mathmatical tricks(put more details about the process in the appendix), we start from a portfolio of dynmaic underlying and static log contract and end up with a portfolio of options weighted in proportion to the reverse of its strike price squared. $$S_{\star}$$ is an arbitrary price, which provides great convenience in numerical calculation and the first few terms other than option price are simply adjustment needed for this flexibility.
