...
Traditionally, managed portfolios are implemented as long-short portfolios formed on observable security characteristics. The choices are abundant in their implementations, we are free to choose between a characteristic soring approach v.s. regression approach, control or not control the cross effect from other factors. Despite all the details, the process can be generalized as a portfolio return representing the factors performance and is constructed as a function of concurrent security return along with company characteristics from the previous period.
