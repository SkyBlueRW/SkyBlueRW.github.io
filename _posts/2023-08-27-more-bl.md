...
The prior extracted from reverse optimization of benchmark portfolio is the basement of the quest. And it makes sence. In a market where most portfolios benchmarked to some reference portfolio. It makes sence to have prior mean consistent with this reference portfolio as a start point. After all it is how performance is contrasted and determined. 

With the basement established, it actually become the 'numeraire' in the forecasts combination. Every other 

Though the prior acts as the , I feel the difference between prior and likelihood are not that large. in the framework they are all simply forecast with uncertainties around. 
