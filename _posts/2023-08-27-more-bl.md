...
Generally we care more about the posterior mean compared to the posterior variance. A mean-variance optimization is much more sensitive to the mean instead of the variance. That's probably the reason why lots of modification proposed on Black-Litterman look for the ratio instead. 

Considering that mean instead of variance play a more import part in a mean variance optimization. It is also possible to ignore the refinement on the variance and focus on the posterior mean with $$\dfrac{\tau}{\alpha}$$ only. Such a treatment is no longer a full scaled Bayesian framework while it still carries lots of the metrits.
